...
# -*- coding: utf-8 -*-
'''
NB note: copied and modified from Mehmet Ercan at https://www.kaggle.com/mehmetbercan/dstl-satellite-imagery-feature-detection/display-a-polygon-class-on-16-band-m/run/759322/notebook
...
def truth_polys(image_id, class_id, W, H):
    x = pd.read_csv('/media/nbrown14/extra_space/data/DSTL_challenge/data/train_wkt_v4.csv')
    rows = x.loc[(x.ImageId==image_id) & (x.ClassType==class_id), 'MultipolygonWKT']
    mp = wkt.loads(rows.values[0])
    grid_sizes = pd.read_csv('/media/nbrown14/extra_space/data/DSTL_challenge/data/grid_sizes.csv', names=['ImageId', 'Xmax', 'Ymin'], skiprows=1)
...
rgbfile='/media/nbrown14/extra_space/data/DSTL_challenge/data/three_band/{}.tif'.format(ImageID)             
rgb = tifffile.imread(rgbfile)
rgb = np.rollaxis(rgb, 0, 3)     

#Read 16band m image
mfile = '/media/nbrown14/extra_space/data/DSTL_challenge/data/sixteen_band/{}_M.tif'.format(ImageID)
...
ax.imshow(x)
plt.savefig(ImageID+"_class"+str(Class)+".png")
